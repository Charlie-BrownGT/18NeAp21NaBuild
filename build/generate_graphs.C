void generate_2D_histogram_from_branch(const char *dirname="/home/guy/software/geant4/geant4-v11.2.2-gdml-mt-install/projects/diamondDetectorResults/testData/", const char *ext=".root") {
	...
	for(int i = 0; i < file_count; i++){
		...
		TLegend *legend = new TLegend(0.75, 0.9, 1.0, 1.0);
		...
		graph->SetTitle(pngFile2);
		graph->SetMinimum(0);
    	graph->SetMaximum(1100); 
		graph->GetXaxis()->SetTitle("Spatial Distribution on X (mm)");
